...
Role logic must be enforced by:

Supabase RLS

Not frontend-only checks

Workflow Model

The system is task-driven.

Do NOT hardcode workflow transitions in UI logic.
...
Each workflow run is identified by:

references.run_ref_id
...
Admin run creation (run_ref_id)

Admin inbox + claim logic

Script approval flow

Guest RAW submission
...
Do not build beyond this unless instructed.
